...
export const taoDanhGia = async (
  ...
  return DanhGia.findById(danhGia._id).populate({
    path: "benhNhanId",
    populate: { path: "nguoiDungId", select: "ten" },
  });
};

// ── Tính lại điểm trung bình (gọi mỗi khi có đánh giá mới) ──────────────────
const tinhLaiDiemTrungBinh = async (bacSiId) => {
  const objectId = new mongoose.Types.ObjectId(bacSiId);
  const ketQua = await DanhGia.aggregate([
    { $match: { bacSiId: objectId } },
    {
      $group: {
        _id: "$bacSiId",
        trungBinh: { $avg: "$soSao" },
...
export const layDanhGiaBacSi = async (
  bacSiId,
  { trang = 1, gioiHan = 10 } = {},
) => {
