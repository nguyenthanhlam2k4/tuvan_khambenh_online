...
export default function TimBacSi() {
    ...
    const [ckChon, setCk]      = useState("Tất cả");
    const [trang, setTrang]    = useState(1);
    const [tongTrang, setTT]   = useState(1);
    const [tongSo, setTS]      = useState(0);

    const tai = useCallback(async (tk = tuKhoa, ck = ckChon, t = 1) => {
        setLoad(true);
        try {
        ...
        <div style={s.page}>
            ...
            <div style={s.body}>
                ...
                <div style={s.content}>
                    ...
                    {loading ? (
                        ...
                        <div style={s.empty}>
                            ...
                            <p style={{ fontSize: 13, color: "#9CA3AF" }}>
                                Thử tìm với từ khoá khác hoặc chọn chuyên khoa khác.
                            </p>
                        </div>
                    ) : (
                        <div style={s.grid}>
                    ...
                    {tongTrang > 1 && (
                        <div style={s.paging}>
                            <button disabled={trang === 1}
                                onClick={() => tai(tuKhoa, ckChon, trang - 1)}
                                style={{ ...s.pageBtn, opacity: trang === 1 ? 0.4 : 1 }}>
                                ← Trước
                            </button>
                            {Array.from({ length: tongTrang }, (_, i) => i + 1).map(p => (
                                <button key={p} onClick={() => tai(tuKhoa, ckChon, p)}
                                    style={{ ...s.pageBtn, ...(p === trang ? s.pageBtnActive : {}) }}>
                                    {p}
                                </button>
                            ))}
                            <button disabled={trang === tongTrang}
                                onClick={() => tai(tuKhoa, ckChon, trang + 1)}
                                style={{ ...s.pageBtn, opacity: trang === tongTrang ? 0.4 : 1 }}>
                                Sau →
                            </button>
                        </div>
                    )}
...
function BacSiCard({ bs, onClick }) {
    ...
    const slotHom = (bs.lichLamViec || [])
        .find(n => n.ngay === new Date().toISOString().slice(0, 10));
    const soSlotTrong = slotHom?.khungGios?.filter(k => !k.daDat).length || 0;
    ...
    return (
        <div style={s.card} onClick={onClick}>
            ...
            <div style={s.cardStats}>
                ...
                <div style={s.statDivider} />
                <StatItem
                    value={soSlotTrong > 0 ? `${soSlotTrong} slot` : "Hết slot"}
                    label="Hôm nay"
                    highlight={soSlotTrong > 0}
                />
            </div>

            {/* Mô tả */}
            {bs.moTa && (
                <p style={s.cardMoTa}>{bs.moTa.slice(0, 80)}{bs.moTa.length > 80 ? "..." : ""}</p>
            )}
...
const s = {
    ...
    heroTitle:    { fontSize: 28, fontWeight: 700, color: "#111", margin: "0 0 8px", letterSpacing: "-0.5px" },
    heroSub:      { fontSize: 14, color: "#6B7280", margin: "0 0 24px" },

    searchBar:    { display: "flex", gap: 8, maxWidth: 560, margin: "0 auto 20px", background: "#fff", border: "0.5px solid #E5E7EB", borderRadius: 12, padding: 6 },
    searchInput:  { flex: 1, height: 38, border: "none", outline: "none", fontSize: 14, color: "#111", padding: "0 10px", background: "transparent" },
    searchBtn:    { height: 38, padding: "0 20px", background: "#1D9E75", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer", flexShrink: 0 },
    ...
    card:         { background: "#fff", border: "0.5px solid #E5E7EB", borderRadius: 14, padding: "16px", cursor: "pointer", transition: "border-color 0.15s", display: "flex", flexDirection: "column", gap: 10 },
    ...
    statDivider:  { width: "0.5px", background: "#E5E7EB", flexShrink: 0 },
