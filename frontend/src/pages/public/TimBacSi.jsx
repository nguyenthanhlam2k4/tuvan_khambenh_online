      ...
      <div style={s.header}>
        <div style={s.headerInner}>
          ...
          <div style={s.headerRight}>
            <button onClick={() => navigate("/dang-nhap")} style={s.btnOutline}>
              Đăng nhập
            </button>
            <button onClick={() => navigate("/dang-ky")} style={s.btnPrimary}>
              Đăng ký
            </button>
          </div>
        </div>
        ...
        <div style={s.content}>
          <div style={s.resultHeader}>
            <span style={s.resultCount}>
              {loading
                ? "Đang tìm..."
                : `${tongSo} bác sĩ${ckChon !== "Tất cả" ? ` chuyên khoa ${ckChon}` : ""}`}
            </span>
          </div>
          ...
          {tongTrang > 1 && (
            <div style={s.paging}>
              ...
              {Array.from({ length: tongTrang }, (_, i) => i + 1).map((p) => (
                <button
                  key={p}
                  onClick={() => tai(tuKhoa, ckChon, p)}
                  style={{
                    ...s.pageBtn,
                    ...(p === trang ? s.pageBtnActive : {}),
                  }}
                >
                  {p}
                </button>
...
function BacSiCard({ bs, onClick }) {
  console.log("======== BAC SI CARD ========");
  console.log("BS:", bs);
  console.log("LICH LAM VIEC:", bs.lichLamViec);

  const ten = bs.nguoiDungId?.ten || "Bác sĩ";
  const soSao = bs.diemDanhGia > 0 ? bs.diemDanhGia.toFixed(1) : null;
  const today = new Date().toLocaleDateString("en-CA");
  const slotHom = (bs.lichLamViec || []).find((n) => n.ngay === today);
  const soSlotTrong = slotHom?.khungGio?.filter((k) => !k.daDat).length || 0;
  return (
    <div style={s.card} onClick={onClick}>
      ...
      <div style={s.cardStats}>
        <StatItem
          value={bs.soNamKinhNghiem > 0 ? `${bs.soNamKinhNghiem} năm` : "—"}
          label="Kinh nghiệm"
        />
        <div style={s.statDivider} />
        <StatItem
          value={soSao ? `${soSao} ★` : "—"}
          label={`${bs.tongDanhGia || 0} đánh giá`}
        />
        <div style={s.statDivider} />
        <StatItem
